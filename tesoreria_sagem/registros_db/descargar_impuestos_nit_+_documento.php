...
function crearPagos($nit,$docto)
	{
		if($docto == NULL or $docto=='') $docto='NULL';
		else $docto="'".$docto."'";

		$consulta="SELECT * FROM tesoreria_global.exportar_impuestos_nit_y_documento('".$nit."',".$docto.");";
...
if($docto == NULL OR $docto== '' ){
	$fieldname = date("Y-m-d")."_IMPUESTOS_NIT_".$nit;
}
else 
	$fieldname = date("Y-m-d")."_IMPUESTOS_NIT_".$nit."_NUMERO_DOCUMENTO_IMPUESTO_".$docto;
