...
function crearCartera($nit,$cta)
	{
		if($cta==NULL or $cta=='') $cta='NULL';
		else $cta="'".$cta."'";

		$consulta="SELECT * FROM tesoreria_global.exportar_cartera_nit_y_numero_cuenta_cobro('".$nit."',".$cta.");";
...
$consulta=crearCartera($_POST['txtnit'],$_POST['txtcta']);	
...
if($cta == NULL OR $cta== ''){
	$fieldname = date("Y-m-d")."_CARTERA_NIT_".$nit;
}
else 
	$fieldname = date("Y-m-d")."_CARTERA_NIT_".$nit."_NUMERO_CUENTA_COBRO_".$cta;
